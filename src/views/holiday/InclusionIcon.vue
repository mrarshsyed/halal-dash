...
const baseUrl = 'admin/holiday/inclusion-icons'
const Form = ref({
  id: null,
  fields: [
...
const saveRating = async () => {
  ...
  if (!Form?.value?.id && !payload?.uploads) {
    store.showSnackbar('Icon is required', 'error')
    return
  }
  ...
  formData.append('uploads', payload.uploads?.[0])
...
const onDelete = async (item) => {
  ...
  const dialogModal = {
    title: 'Delete Icon',
    content: 'Are you sure to delete icon?',
    confirmText: 'Delete',
    formComponents: {},
